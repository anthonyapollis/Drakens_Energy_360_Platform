-- =====================================================================
-- Vivo Energy 360 - Unity Catalog governance
--
...
--   vivo_platform_engineers - own the pipelines
--   vivo_data_analysts      - read gold, masked PII
--   vivo_commercial_team    - read gold + unmasked customer names
--   vivo_data_scientists    - read silver + gold, write ml schema
--   vivo_executives         - read the executive aggregates only

GRANT USE CATALOG ON CATALOG vivo_${env} TO `vivo_platform_engineers`;
GRANT USE CATALOG ON CATALOG vivo_${env} TO `vivo_data_analysts`;
GRANT USE CATALOG ON CATALOG vivo_${env} TO `vivo_commercial_team`;
GRANT USE CATALOG ON CATALOG vivo_${env} TO `vivo_data_scientists`;
GRANT USE CATALOG ON CATALOG vivo_${env} TO `vivo_executives`;

-- Engineers own bronze and silver outright.
GRANT ALL PRIVILEGES ON SCHEMA bronze TO `vivo_platform_engineers`;
GRANT ALL PRIVILEGES ON SCHEMA silver TO `vivo_platform_engineers`;
GRANT ALL PRIVILEGES ON SCHEMA gold TO `vivo_platform_engineers`;
GRANT ALL PRIVILEGES ON SCHEMA platform TO `vivo_platform_engineers`;
...
-- dashboard.
GRANT USE SCHEMA, SELECT ON SCHEMA gold TO `vivo_data_analysts`;
GRANT USE SCHEMA, SELECT ON SCHEMA platform TO `vivo_data_analysts`;

GRANT USE SCHEMA, SELECT ON SCHEMA gold TO `vivo_commercial_team`;

-- Scientists need silver for feature engineering and own the ml schema.
GRANT USE SCHEMA, SELECT ON SCHEMA silver TO `vivo_data_scientists`;
GRANT USE SCHEMA, SELECT ON SCHEMA gold TO `vivo_data_scientists`;
GRANT ALL PRIVILEGES ON SCHEMA ml TO `vivo_data_scientists`;

-- Executives see only the pre-aggregated tables, not transaction detail.
GRANT USE SCHEMA ON SCHEMA gold TO `vivo_executives`;
GRANT SELECT ON TABLE gold.agg_executive_daily_kpi TO `vivo_executives`;
GRANT SELECT ON TABLE gold.network_investment_scorecard TO `vivo_executives`;
...
RETURN CASE
    WHEN is_account_group_member('vivo_commercial_team') THEN name
    WHEN is_account_group_member('vivo_platform_engineers') THEN name
...
RETURN CASE
    WHEN is_account_group_member('vivo_commercial_team') THEN amount
    WHEN is_account_group_member('vivo_platform_engineers') THEN amount
    ELSE NULL
END;

CREATE OR REPLACE FUNCTION gold.mask_loyalty_member(member_id STRING)
RETURN CASE
    WHEN is_account_group_member('vivo_platform_engineers') THEN member_id
    -- Analysts get a stable pseudonym: still joinable and countable, but not
    -- resolvable back to a person.
    ELSE sha2(concat(member_id, 'vivo360-pseudonym-salt'), 256)
END;
