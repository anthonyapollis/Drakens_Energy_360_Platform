...
FINDINGS = [
    ...
    ("A rule inferred from a column name, four times over",
     "The pattern recurred often enough to be worth naming. A validity rule "
     "written against what a column is <em>called</em> rather than what it "
     "<em>means</em> destroys real data, and does it silently while every "
     "model builds and every test passes. `signed_amount_zar >= 0` "
     "quarantined every credit leg of the general ledger. A 2023-2027 "
     "timestamp window nulled 89% of site opening dates. An 8-45 rand fuel "
     "price band rejected 36% of the shop feed, where the same column name "
     "carries the price of a pie. Requiring a site on every row quarantined "
     "58% of digital events, which are app sessions and were never at a "
     "station. Bands are now keyed by table and column, attribute dates get a "
     "window wide enough for a business history, and optional foreign keys "
     "are measured rather than assumed."),
    ("A tab that split a dimension in two",
     "The maintenance heatmap showed thirteen asset types where the dimension "
     "has twelve. clean_text called trim(), which in both DuckDB and Spark "
     "removes spaces and nothing else, so a tab-padded value reached gold as "
     "a separate category from the same value without one. Both real, both "
     "counted separately, neither obviously wrong in a total."),
    ("Observability that measured 8% of what it claimed",
     "obs_cleansing_summary named fourteen tables against a landing zone of "
     "167, and the fourteen were whichever tables someone had thought to add "
     "— the least useful sample available. It is now generated by the same "
     "pass that generates the cleansing models: 128 monitored tables, "
     "2,049,277 raw rows reconciling exactly to cleansed plus quarantined "
     "plus deduplicated, and zero tables failing reconciliation."),
