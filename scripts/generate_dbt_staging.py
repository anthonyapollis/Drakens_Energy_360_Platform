...
# Plausible bands for unit prices, keyed by table *and* column.
#
# A regulated South African fuel price has never been R1.51 or R0.00 a litre,
# so a row claiming one has lost a decimal separator or picked up a sentinel
# and must not reach a margin calculation. But the first version of this rule
# was keyed on the column name alone, and `unit_price_zar` is not a fuel price
# everywhere it appears: on the shop feed it is the price of a pie or a
# newspaper. The fuel band rejected 41,645 shop lines -- 36% of the table --
# every one of them a perfectly good row.
#
# That is the third time in this project a rule inferred from a column's name
# has destroyed real data, after the signed ledger amounts and the site
# opening dates. The pattern is consistent enough to be worth stating: a
# column name tells you what a value is called, not what it means, and the
# meaning is what a validity rule has to be written against.
PRICE_BANDS = {
    ("fact_retail_fuel_sales", "unit_price_zar"): (8.0, 45.0),
    ("fact_retail_fuel_sales", "price_per_litre_zar"): (8.0, 45.0),
    ("fact_wholesale_fuel_sales", "unit_price_zar"): (8.0, 45.0),
    # Shop, LPG and lubricant lines span a genuinely wide range and get only
    # a sanity ceiling, not a fuel band.
    ("fact_shop_sales", "unit_price_zar"): (0.0, 5_000.0),
    ("fact_lpg_sales", "unit_price_zar"): (0.0, 10_000.0),
    ("fact_lubricant_sales", "unit_price_zar"): (0.0, 20_000.0),
}

# Foreign keys that are optional by design, measured rather than assumed:
# these columns are null far more often than the injector's null rate, which
# is the generator saying the column does not apply to this table.
#
#   fact_digital_events.site_id       57.8% -- an app session is not at a site
#   fact_inventory_snapshot.site_id   25.2% -- terminal stock is not site stock
#   fact_lpg_sales.customer_id        65.3% -- walk-in cash sales have no account
#   fact_lubricant_sales.customer_id  53.7% -- the same
#
# Requiring them quarantined 34,808 digital events, 58% of the feed, for
# lacking a station they were never at.
OPTIONAL_FOREIGN_KEYS = {
    ("fact_digital_events", "site_id"),
    ("fact_digital_sessions", "site_id"),
    ("fact_inventory_snapshot", "site_id"),
    ("fact_lpg_sales", "customer_id"),
    ("fact_lubricant_sales", "customer_id"),
}
...
def quality_rules(table: str, columns: list[str], types: dict,
    ...
    for col in columns:
        ...
        if (table, col) in PRICE_BANDS:
            lo, hi = PRICE_BANDS[(table, col)]
...
def render_monitored_tables(monitored: list[tuple[str, str]]) -> str:
    """A macro returning the tables `obs_cleansing_summary` counts.

    Generated rather than hand-kept, and returned from a macro rather than
    derived from `graph` inside the model.

    Deriving it from the graph is the obvious approach and it does not work:
    `graph` is only populated at execute time, so every `ref()` built from it
    sits inside a conditional and dbt cannot infer the model's dependencies at
    parse time. It fails with a compilation error naming one arbitrary missing
    dependency, which is a confusing way to learn the approach is unavailable.
    A macro is evaluated during parsing, so the refs built from its result are
    static and dbt resolves them normally.

    Generating it keeps the list honest: it is produced by the same pass that
    produced the models it names, so a new fact table enters the observability
    the moment its cleansing models exist. The fourteen-table list this
    replaced covered 8% of the landing zone, and the eight percent was
    whichever tables someone had thought to add.
    """
    lines = [
        "{#-",
        "    GENERATED by scripts/generate_dbt_staging.py -- do not edit by hand.",
        ...
        "    {#-",
        f"        The {len(monitored)} tables with both a staging and a quarantine",
    ...
    for source_name, table in monitored:
        lines.append(f"        ('{source_name}', '{table}'),")
    lines += [
        "    ] %}",
        "    {{ return(tables) }}",
        "{% endmacro %}",
...
def main(argv=None):
    ...
    for table, info in sorted(tables.items()):
        ...
        if quality_rules(table, cols, types, pick_key(table, cols)):
            ...
            n_qtn += 1
            monitored.append(
                ("lake_facts" if layer == "facts" else "lake_dimensions",
                 table))
    ...
    macro_path = Path("dbt/macros/monitored_tables.sql")
    macro_path.parent.mkdir(parents=True, exist_ok=True)
    macro_path.write_text(
        render_monitored_tables(sorted(monitored)), encoding="utf-8")

    print(f"wrote {n_cln} repair/assess models to {args.cleansed_out}")
    print(f"wrote the {len(monitored)}-table monitored list to {macro_path}")
